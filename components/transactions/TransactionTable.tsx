...
const TransactionIdCell: React.FC<{ id: string; date: string }> = ({ id, date }) => (
  <td className="px-6 py-4">
    <p className="text-sm font-bold text-blue-600 truncate max-w-[120px]">{id}</p>
    <p className="text-[10px] text-gray-400 font-medium">{new Date(date).toLocaleString()}</p>
  </td>
);

const ProductCell: React.FC<{ productName?: string; providerName?: string }> = ({ productName, providerName }) => (
  <td className="px-6 py-4">
    <span className="text-sm font-semibold text-gray-800">{productName || 'Mixed Items'}</span>
...
const TransactionRow: React.FC<{ transaction: Transaction }> = ({ transaction }) => {
  ...
  return (
    <tr className="hover:bg-gray-50/50 transition cursor-default">
      <TransactionIdCell id={transaction.TransactionId} date={transaction.TransactionDateTime} />
      <td className="px-6 py-4 text-sm font-medium text-gray-600">
        {new Date(transaction.AccessDateTime).toLocaleDateString()}
      </td>
      <ProductCell productName={firstProduct?.ProductName} providerName={firstProduct?.ProviderName} />
